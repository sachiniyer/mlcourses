"""
Simple machine learning functions for the homework.

Homework 0 of DLSYS course
"""
...
def add(x, y):
    """
    Trivial 'add' function.

    You should implement to get used to the autograder and submission system.
    The solution to this problem is in the homework notebook.
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    for i in range(X.shape[0] // batch):
        ...
        grad = X_batch.T @ (
            exp_Z / np.sum(exp_Z, axis=1)[:, None] - np.eye(exp_Z.shape[1])[y_batch]
        )
        theta -= lr * grad / batch
    ### END YOUR CODE
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for i in range(X.shape[0] // batch):
        ...
        Z2_batch = A1_batch @ W2
        exp_Z2 = np.exp(Z2_batch)
        grad_W2 = A1_batch.T @ (
            exp_Z2 / np.sum(exp_Z2, axis=1)[:, None] - np.eye(exp_Z2.shape[1])[y_batch]
        )
        grad_W1 = X_batch.T @ (
            (
                exp_Z2 / np.sum(exp_Z2, axis=1)[:, None]
                - np.eye(exp_Z2.shape[1])[y_batch]
            )
            @ W2.T
            * (Z1_batch > 0)
        )
