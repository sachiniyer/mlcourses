...
def ConvBN(
    in_channels, out_channels, kernel_size=3, stride=1, device=None, dtype="float32"
):
...
class ResidualBlock(ndl.nn.Module):
    def __init__(
        ...
        self.conv2 = ConvBN(
            out_channels, out_channels, kernel_size, stride, device=device, dtype=dtype
        )
...
class ResNet9(ndl.nn.Module):
    def __init__(self, device=None, dtype="float32"):
        ...
        self.conv4 = ConvBN(
            64, 128, kernel_size=3, stride=2, device=device, dtype=dtype
        )
